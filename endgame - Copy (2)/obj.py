        ...
        while True:
            ...
            if "stop" in request:
               ...
               if os.path.exists(ppt_file):
                ...
                city = request.replace("weather in", "").strip()
                get_weather(city)

            elif "define" in request:
                word = request.replace("define", "").strip()
                definition = get_definition(word)
                speak(definition)

            elif "convert currency" in request:
             try:
                # Ensure the input follows the format "convert currency <amount> <from_currency> to <to_currency>"
                parts = request.replace("convert currency", "").strip().split("to")

                if len(parts) == 2:  # Only proceed if "to" is present in the input
                    amount_currency = parts[0].strip().split()  # Split amount and from_currency
                    amount = float(amount_currency[0])  # Convert the amount to float
                    from_currency = amount_currency[1]  # The first currency
                    to_currency = parts[1].strip()  # The second currency
                    ...
                    speak("Please provide the correct format for currency conversion. Example: 'convert currency 100 USD to EUR'")
             ...
             try:
        # Split the request into parts, expecting a format like "translate <text> to <language>"
                parts = request.split("to")
                if len(parts) == 2:
                   text = parts[0].replace("translate", "").strip()  # Get the text to translate
                   target_language = parts[1].strip()  # Get the target language

                   if text and target_language:  # Ensure both parts are valid
                ...
                # Print the translation to the console
                      ...
                      speak("I didn't get a valid text or language to translate.")
                else:
                  speak("Please follow the correct format: 'translate <text> to <language>'")
    
             except Exception as e:
               speak(f"Sorry, I couldn't process the translation. Error: {e}")
               print(f"Error: {e}")

            elif "news on" in request:
             topic = request.replace("news on", "").strip()
             if topic:
                 get_news_by_topic(topic)
             else:
                 speak("Please specify a topic. For example, you can say 'news on technology'.") 

            elif "bitcoin price" in request:
                 get_bitcoin_price()  # Call the function to get the Bitcoin price                    
...
class FaceAuthenticationWindow(QtWidgets.QMainWindow):
    ...
    def launch_jarvis(self):
        ...
        subprocess.Popen(["python", "obj.py"])
